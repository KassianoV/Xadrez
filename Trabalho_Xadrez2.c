...
int CalculaCavalo(struct Posicao PosAtual, int linha, int coluna, struct Jogada *listaJogada){
    ...
    if (linha<7){//uma linha acima
        if (coluna<6){
					if (PosAtual.tab[linha+1][coluna+2] == NULL ||  PosAtual.tab[linha+1][coluna+2]->codigo*peca<=0){ //duas casas à direita
        ...
        if (coluna>1){
					 if(PosAtual.tab[linha+1][coluna-2] == NULL || PosAtual.tab[linha+1][coluna-2]->codigo*peca<=0){ //duas casas à esquerda
            listaJogada = CriaJogada(linha,coluna,linha+1,coluna-2); qtd++;
						if(PosAtual.tab[linha+1][coluna-2] != NULL)
...
void EscolhePeca(struct Posicao PosAtual,int *linha, int *coluna){
    do{
        ...
        scanf("%d",linha);
				printf("coluna:");
        scanf("%d",coluna); //lê valores de 1 a 8
