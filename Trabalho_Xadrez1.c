...
//1 - Uma função IniciaTabuleiro (int tab[8][8]) que receba um tabuleiro e coloque as peças na posição inicial, de acordo com as explicações acima.
...
/*2 - Uma função que mostre o tabuleiro na tela, de forma “semelhante” à figura acima (mostrando os códigos no lugar das peças). Pode ser utilizado o sistema de coordenadas da matriz (linhas e colunas entre 0 e 7).*/
...
int main() {
  ...
  while(1){
    PrintTab(Tab);
  //3.1- Pergunta ao usuario qual peça ele deseja mover por meio da linha e da coluna em que a peça está localizada.
    printf("Qual peça deseja mover: (ex: linha 1, coluna 3)");
    scanf("%d %d", &movimento.linhaDE, &movimento.colunaDE);

  //3.2- Analisa se a posição possui alguma peça ou se existe. Caso aconteça, pede ao usuário que informe uma nova peça.
    // peão:
...
*/
	/* O programa deve permitir a captura de peças, retirando a peça capturada da posição em que estava e colocando ali a peça que fez a captura. O programa encerra quando um rei for capturado e deve informar quem foi o jogador vencedor. A captura só pode ser feita por uma peça do jogador adversário, nunca por outra peça do mesmo jogador.*/
